...
import wiley.automation.core.util.ObjectRepository;
...
public static boolean searchResultVerify(String string){
	...
	public static boolean goToAdminProfilePage(String customer) {
		// TODO Auto-generated method stub
		if(CommonUtil.isElementPresent(InstituteLogin)){
			/*if(EnvUtil.getProperty("Browser").equalsIgnoreCase("ie"))
				CommonUtil.doubleClick(ObjectRepository.InstituteLoginxpath);
			else*/
			CommonUtil.click(InstituteLogin);
			if(!CommonUtil.isElementDisplay(InstitutionalAdmin)){
				CommonUtil.doubleClick(ObjectRepository.InstituteLoginxpath);
			}
			CommonUtil.sleep(2000);
			if(CommonUtil.isElementPresent(InstitutionalAdmin)){
				CommonUtil.click(InstitutionalAdmin);
				msg = "Institutional administration page loaded properly";	
				CommonUtil.report(msg,pass);
				return true;
			}else{
				msg = "Institutional administration page not loaded properly";	
				CommonUtil.report(msg,fail);
	...
	public static String verifyTokenCount(String licenseID) {
		// TODO Auto-generated method stub
		CommonUtil.clicksync(ArticleSelectxpath);
		
		if(CommonUtil.getText(LicenseID_Lit).contains(licenseID)){
			String s = CommonUtil.getText(Token_Lit);
			return s;
		}
