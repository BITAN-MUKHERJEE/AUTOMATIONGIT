...
public class CompareTwoImages {
	...
	public boolean compare() {
		...
		int blocksX1 = (int)(image2.getWidth()/compareX);
		int blocksY1 = (int)(image2.getHeight()/compareY);
//		int blocksX=44;
//		int blocksY=25;
		CompareTwoImages.isIdentic = true;
		if(blocksX==blocksX1 && blocksY==blocksY1){
			for (int y = 0; y < compareY; y++) {
				for (int x = 0; x < compareX; x++) {
					int result1 [][] = convertTo2D(image1.getSubimage(x*blocksX, y*blocksY, blocksX, blocksY));
					int result2 [][] = convertTo2D(image2.getSubimage(x*blocksX, y*blocksY, blocksX, blocksY));
					for (int i = 0; i < result1.length; i++) {
						for (int j = 0; j < result1[0].length; j++) {
							...
							if (diff/Math.abs(result1[i][j]) > sensitivity) {
								...
								g2.drawRect(x*blocksX, y*blocksY, blocksX, blocksY);
								isIdentic = false;
							}
						}
					}
